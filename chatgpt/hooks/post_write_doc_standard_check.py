...
# «всё чисто». Самодиагностика этого не видит: она проверяет, что хук
# запускается, а не что его область совпала с реальностью.
#
#   IVAN_ENTITY_DIRS — имена папок через запятую; заданы — заменяют
#                      набор по умолчанию целиком (человек знает свою
#                      раскладку лучше нас).
# Образец — post_write_md_check (IVAN_MD_*), там же и мотивация.
def _entity_dirs():
    raw = os.environ.get("IVAN_ENTITY_DIRS", "")
    names = [x.strip("/ ") for x in raw.split(",") if x.strip(" /")]
    if not names:
        names = ["decisions", "scope", "concepts", "specs"]
...
def is_target_path(file_path: str) -> bool:
    ...
    if "/instructions/workflows/" in norm or "/instructions/reference/" in norm:
        return False
    return any(seg in norm for seg in _entity_dirs())
...
def main() -> int:
    ...
    if not has_h2(body, "Словарь", "Glossary"):
        violations.append(
            ...
            "      Как поправить: добавить в начало текста (после шапки) блок:\n"
            "        ## Словарь\n"
    ...
    if doc_type in ("decision", "scope") and not has_h2(
        ...
        violations.append(
            ...
            "      Как поправить: добавить блок с описанием — что было ДО решения, "
    ...
    if hybrids:
        ...
        violations.append(
            ...
            "      Как поправить: либо не склонять («pipeline»), либо "
    ...
    if bad_abbrevs:
        violations.append(
            ...
            "      Как поправить: при первом упоминании дать расшифровку в скобках — "
            "«ИИ (искусственный интеллект)», ИЛИ добавить термин в "
            "секцию «## Словарь»."
        )
    ...
    header = (
        f"Документ {os.path.basename(file_path)} помечен готовым, но "
        f"не дооформлен по стандарту."
    )
    if lazy:
        header = (
            f"Документ {os.path.basename(file_path)} помечен готовым, "
            f"но заведён давно — показываю, что не по стандарту, "
            f"не останавливая работу."
        )

    bullets = "\n".join(f"  - {v}" for v in violations)
    footer = (
        f"\nСтандарт целиком — knowledge/document-writing-standards.md.\n"
        f"Спросят подробности или как отключить — это проверка "
        f"стандарта оформления, файл hooks/post_write_doc_standard_check.py."
    )
    msg = f"{header}\nНарушения:\n{bullets}{footer}"
