#!/usr/bin/env python3
"""SessionStart: состояние файлов проверок качества — не факт их работы.

Говорит человеческим языком и один раз за сессию. Различает ступени:
файлы на месте → объявлены платформе → (подтверждены — не видно отсюда) →
(сработали на событии — проверяется только событием). То, чего не видно,
называется неизвестным, а не рабочим.
"""
import json
import os
import sys
...
CAPABILITY = {
    ...
    "post_write_principles_read_check": "проверка чтения принципов",
    "check_deferred_actions": "проверка отложенного",
    "check_closing_handoff": "проверка завершающего вопроса",
...
COMPANIONS = ("_host_adapter.py", "_transcript_io.py", "_support_lookup.py")

# Утилиты git: лежат в hooks/, но событием платформы не запускаются и в
# конфиге не объявлены по природе. Без этой оговорки хук называл сканер
# секретов «приехал, но не включён» (08.09.2026).
GIT_TOOLS = ("secret-scan.py",)
...
def _runs_by_name(hooks_dir):
    ...
    Имя разрешается через PATH, и на Windows может привести к заглушке
    магазина — проверки замолкают. Каталог установленного плагина у Codex
    только для чтения, подставить путь в команды нельзя; единственный способ
    узнать, запускается ли проверка, — живое событие.

    На Mac имя разрешается исправно, поэтому это не тревога, а оговорка.
    """
    import re
    path = os.path.join(hooks_dir, "hooks.json")
    try:
...
def main():
    ...
    if working:
        ...
        # Наличие файла и запись в конфиге — состояние диска, не факт
        # запуска. Прежняя редакция писала «работают» ровно по этим двум
        # признакам; на живой установке Codex 08.09.2026 это дало «всё
        # работает» при неподтверждённых проверках.
        lines.append("Файлы проверок на месте и объявлены платформе: "
                     + ", ".join(names) + ". Это состояние файлов, не факт "
                     "запуска: подтверждены ли они и срабатывают ли на "
                     "событии, отсюда не видно.")
        if "session_start_load_rules.py" in working:
            lines.append("Отдельная оговорка про «загрузку правил при "
                         "старте»: выше проверен только сам файл-хук, не "
                         "то, прочитал ли он СВОЙ файл правил "
                         "(skills/ivan-rules/SKILL.md) и попал ли текст в "
                         "контекст. Если правила не звучат в поведении — "
                         "это не «проверка сломана», это отдельный отказ "
                         "внутри неё же, и хук при нём сам скажет об этом "
                         "явно (не молчанием).")
    ...
    if by_name and working:
        lines.append("Команды запуска зовут интерпретатор по имени "
                     "(python3 / python), не по пути. На Mac имя разрешается; "
                     "на Windows может ответить заглушка магазина, и проверка "
                     "молча не запустится. Узнаётся только живым событием: "
                     "документ без шапки должен получить замечание.")

    if False and working:
        lines.append("В этой среде проверки не запускаются, пока человек их "
                     "не подтвердил в настройках хуков, и об этом среда не "
                     "сообщает. Подтверждены ли — отсюда не видно. "
                     "Подтверждение слетает после каждого обновления.")

    # Общее место поддержки — раз в сессию, если есть сеть: что известно про
    # эту версию обвязки у других получателей (ADR первой линии 2026-09-08).
    # Отказ сети — словами с причиной, не молчанием: молчание здесь означало
    # бы «дефектов нет», а это не проверено.
    try:
    ...
    context = ("# Состояние проверок качества\n\n" + "\n\n".join(lines)
               ...
               "одной фразой, его словами, без имён файлов. Ничего не "
               "сломано — молчи. Спросит подробности — назови поимённо, "
               "там имена уместны.\n\nСпросит «работают ли проверки» — "
               "отвечай по ступеням, не сводя к одному слову: файлы на месте "
               "· объявлены платформе · подтверждены (не видишь) · сработали "
               "на событии (видно только по событию: попроси создать "
               "документ без шапки и жди замечания). «Всё работает» не "
               "говори, пока событие не наблюдалось.\n\nЧего эта проверка "
               "не видит: она сама запущена тем же способом, что и "
               "остальные, и доказывает лишь, что способ запуска для неё "
               "сработал. Не запустился бы способ — не было бы и этого "
               "сообщения.")
